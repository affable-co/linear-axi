 ...
 * Adapted from gh-axi (MIT, Kun Chen).
 *
...
export function cleanBody(text: string): string {
  // Normalize Linear issue URLs to bare identifiers:
  // https://linear.app/<workspace>/issue/ABC-123[/slug] → ABC-123
  let s = text.replace(
    /\[([^\]]+)\]\(https:\/\/linear\.app\/[^/]+\/issue\/([A-Za-z0-9]+-\d+)[^)]*\)/g,
    "[$1]($2)",
  );
  s = s.replace(/(?<!\()https:\/\/linear\.app\/[^/]+\/issue\/([A-Za-z0-9]+-\d+)\S*/g, "$1");
 ...
 */
export function truncateBody(body: unknown, maxLen = 500, options: TruncateBodyOptions = {}): string {
