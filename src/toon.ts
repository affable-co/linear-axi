...
/**
 * Field extractor definitions for transforming Linear API JSON into flat
 * TOON-friendly objects. Adapted from gh-axi (MIT, Kun Chen).
 */
...
/** Linear priority numbers → names. 0 means "no priority". */
...
/** Extractor for Linear's numeric `priority` field. */
export function priorityName(key = "priority", as = "priority"): FieldDef {
  return custom(as, (item) => PRIORITY_NAMES[item[key] as number] ?? "none");
}
  ...
  for (const def of schema) {
    ...
    switch (def.type) {
      ...
      case "pluck":
        ...
        break;
      case "joinArray": {
        // Accepts plain arrays or Linear connection shapes ({ nodes: [...] }).
        const raw = item[def.key];
        const arr = Array.isArray(raw) ? raw : (raw?.nodes as unknown[] | undefined);
        if (Array.isArray(arr) && arr.length > 0) {
          result[outputKey] = arr
            .map((x: unknown) => (typeof x === "string" ? x : (x as Record<string, unknown>)[def.subkey]))
            .join(",");
        } else {
      ...
      case "mapEnum": {
        ...
        if (typeof strVal === "string" && strVal !== "" && strVal in def.map) {
          result[outputKey] = def.map[strVal];
        } else {
