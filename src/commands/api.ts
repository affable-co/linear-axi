...
description: Raw Linear GraphQL escape hatch for queries the dedicated commands do not cover (prefer \`issue create/update --blocked-by/--blocks/...\` for relations). Prints the unwrapped data payload as compact JSON (no {"data":…} wrapper, no TOON). Query may also be piped on stdin. Issue/team identifiers (ABC-123) are accepted anywhere Linear accepts them.
flags[1]:
...
examples:
  ...
  linear-axi api '{ organization { id name urlKey } }'
`;
